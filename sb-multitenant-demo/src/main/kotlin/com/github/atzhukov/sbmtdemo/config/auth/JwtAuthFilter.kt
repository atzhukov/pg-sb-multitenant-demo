...
class JwtAuthFilter(
	...
	private val userWithDetailsService: UserWithDetailsService
	...
	override fun doFilterInternal(
		...
		if (header != null && header.startsWith("Bearer ")) {
			try {
				...
				val jwtAuth = jwtService.parseToken(token)
				if (SecurityContextHolder.getContext().authentication == null) {
					val user = userWithDetailsService.loadUserByUsername(jwtAuth.username)
					if (user.username == jwtAuth.username) {
						jwtAuth.details = WebAuthenticationDetailsSource().buildDetails(request)
